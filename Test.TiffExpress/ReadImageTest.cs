...
namespace Test.TiffExpress
{
    public class ReadImageTest
    {
        ...
        {
            using(var tiffReader = new TiffReader(File.OpenRead(Path.Combine("TestData", "gray8.tif"))))
            {
        ...
        {
            using(var tiffReader = new TiffReader(File.OpenRead(Path.Combine("TestData", "gray16.tif"))))
            {
                var ifd = tiffReader.ImageFileDirectories[0];
                Assert.Equal(64, ifd.ImageWidth);
                Assert.Equal(64, ifd.ImageHeight);
        ...
        {
            using(var tiffReader = new TiffReader(File.OpenRead(Path.Combine("TestData", "gray16_msb.tif"))))
            {
                var ifd = tiffReader.ImageFileDirectories[0];
                Assert.Equal(64, ifd.ImageWidth);
                Assert.Equal(64, ifd.ImageHeight);
